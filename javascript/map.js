...
const cityRegions = [
    ...
    { id: 'pr-id', label: 'Isla Desecheo', lat: 18.383861, lng: -67.480984, region: 'West' },
...
function getCityInfo(region) {
    const cityInfo = typeof mapCityInfo !== 'undefined' ? mapCityInfo[region.id] : null;
    let summary = 'More information for this city can be added in data.js.';
    
    if (cityInfo) {
        // Use the appropriate language translation
        if (cityInfo[currentLanguage]) {
            summary = cityInfo[currentLanguage];
        } else if (cityInfo.english) {
            // Fall back to English if the current language isn't available
            summary = cityInfo.english;
        }
    }

    return `
        <span class="city-info-intro">${summary}</span>
        <span class="city-region-line"><strong>${translations[currentLanguage].mapRegionLabel || 'Region'}:</strong> ${region.region}</span>
    `;
}

function setActiveRegionButton(regionId) {
    document.querySelectorAll('#map-region-buttons button').forEach(button => {
        button.classList.toggle('active', button.dataset.regionId === regionId);
    });
}
...
function updateMapInfo(region) {
    ...
    if (descEl) {
        descEl.innerHTML = getCityInfo(region);
    }
...
function initializeMap() {
    ...
    map = L.map(mapContainer).setView([18.2208, -66.5901], 8);
    ...
    cityRegions.forEach(region => {
        ...
        const marker = L.marker([region.lat, region.lng], {
            icon: markerIcon,
            title: region.label
        }).bindPopup(`<strong>${region.label}</strong><br>`);

        // Click event to show info and track city visit
        marker.on('click', () => {
            updateMapInfo(region);
            markCityVisited(region.id);
        });
...
function createRegionButtons() {
    ...
    // Create a button grid to hold all the city buttons
    const buttonGrid = document.createElement('div');
    buttonGrid.style.display = 'grid';
    buttonGrid.style.gridTemplateColumns = 'repeat(auto-fit, minmax(120px, 1fr))';
...
function loadMapContent() {
    ...
    if (nameEl) {
        nameEl.innerText = lang.mapDefaultName || 'Explore Puerto Rico';
    }

    if (descEl) {
        descEl.innerText = lang.mapLoadDescription || 'Discover the city regions on the Puerto Rico map';
    }
}
...
// This is the key used when saving the quiz state in localStorage.
...
// This stores the shuffled question order so it persists across page reloads.
let quizQuestionOrder = [];

// Save the current quiz state to localStorage.
function saveQuizState() {
    // Store question IDs to preserve the order
    const questionIds = [];
    for (let i = 0; i < currentQuizQuestions.length; i++) {
        questionIds.push(currentQuizQuestions[i].id);
    }
...
// Clear the saved quiz state (called when retaking the quiz).
...
// Start a fresh quiz attempt or restore a saved one.
// This randomizes the questions, limits how many are used,
// and clears out any old answers from the last attempt.
// If a saved quiz state exists, it restores that instead.
function initializeQuiz(restoreSavedState = true) {
    // Check if we should restore a saved quiz state.
    if (restoreSavedState) {
        const savedState = loadQuizState();
        if (savedState && savedState.questionIds && savedState.questionIds.length > 0) {
            // Get all questions in the current language and preserve the saved order by id.
            const allQuestions = getQuizQuestions();
            const restoredQuestions = [];
            
            for (let i = 0; i < savedState.questionIds.length; i++) {
                const questionId = savedState.questionIds[i];
                for (let j = 0; j < allQuestions.length; j++) {
                    if (allQuestions[j].id === questionId) {
                        restoredQuestions.push(allQuestions[j]);
                        break;
                    }
                }
            }
            
            // Only restore if we found all the questions
            if (restoredQuestions.length === savedState.questionIds.length) {
                currentQuizQuestions = restoredQuestions;
                currentQuizQuestion = savedState.currentQuestion;
...
function loadQuizContent() {
    ...
    // Try to restore a saved quiz, or start fresh.
    initializeQuiz();
    ...
    // Show the current question (or first if starting fresh).
